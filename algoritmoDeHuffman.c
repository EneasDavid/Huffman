...
void imprimirArvoreHuffmanFormatada(No *raiz){
    if (raiz==NULL) return;
    printf("(");
    if (raiz->esquerda==NULL && raiz->direita==NULL) printf("%c", raiz->caractere);
    else{
...
void imprimirArvoreHuffman(No *raiz, int tamanho){
    ...
    if (raiz->esquerda==NULL && raiz->direita==NULL) printf("Caractere: %c\tFrequencia: %u\n", raiz->caractere, raiz->frequencia);
    else{
...
char* decodificar(unsigned char *entrada, No *raiz) {
    char *decodificado=(char*)calloc(strlen((char*)entrada)+1, sizeof(char)); //+1 para o caractere nulo
    testeMalloc(decodificado, "decodificar");
    No *atual=raiz;
    char temporario[2];
    temporario[1]='\0'; // Define o terminador nulo apenas uma vez

    while (*entrada!='\0') {
        if(*entrada=='0') atual=atual->esquerda;
        else atual=atual->direita;
        /*
        se o valor atual for 0 ele deve ir para a esquerda da arvore de huffman,
        se o valor atual for 1 ele deve ir para a direita da arvore de huffman
        e em caso de esquerdo e direito da no atual serem nulos, chegamos em uma folha
        e essa é a letra que queremos
        */
        if (atual->esquerda==NULL && atual->direita==NULL) {
            temporario[0]=atual->caractere;
